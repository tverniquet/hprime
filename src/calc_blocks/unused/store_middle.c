...
int
store_middle_init(struct prime_ctx *pctx, uint32_t start_prime, uint32_t end_prime, void **ctx)
{
   struct store_middle_ctx *sctx = malloc(sizeof (struct store_middle_ctx));
   *ctx = sctx;
...
int
store_middle_free(void *ctx)
{
   struct store_middle_ctx *sctx = ctx;
   FREE(sctx->primelist);
   FREE(sctx->offsets);
   FREE(ctx);
   return 0;
...
int
store_middle_add_sieving_primes(uint32_t *primelist, uint32_t *ind, uint32_t size, void *ctx)
{
   struct store_middle_ctx *sctx = ctx;
   for (; *ind < size; (*ind)++) {
...
int
store_middle_skip_to(struct prime_ctx *__attribute__((unused))pctx, uint64_t __attribute__((unused))target_num, void *__attribute__((unused))ctx)
{
   struct store_middle_ctx *sctx = ctx;
...
static void
check_new_sieve_primes(struct store_middle_ctx *sctx, struct prime_current_block *pcb)
{
   int bit;
...
int
store_middle_calc_primes(struct prime_current_block *pcb, void *ctx)
{
   struct store_middle_ctx *sctx = ctx;
   ...
   for (i = 0; i < sctx->calculated_index; i++)
      compute_block(pcb, sctx->primelist[i].prime, &sctx->offsets[i*8], sctx->primelist[i].n);
