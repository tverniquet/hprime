...
int
lower_upper_init(struct prime_ctx *pctx, uint32_t start_prime, uint32_t end_prime, void **ctx)
{
   struct lower_upper_ctx *sctx = malloc(sizeof (struct lower_upper_ctx));
   *ctx = sctx;
...
int
lower_upper_free(void *ctx)
{
   struct lower_upper_ctx *sctx = ctx;
   FREE(sctx->primelist);
   FREE(sctx->offsets);
   FREE(ctx);
   return 0;
...
int
lower_upper_add_sieving_primes(uint32_t *primelist, uint32_t *ind, uint32_t size, void *ctx)
{
   struct lower_upper_ctx *sctx = ctx;
   for (; *ind < size; (*ind)++) {
...
int
lower_upper_skip_to(struct prime_ctx *__attribute__((unused))pctx, uint64_t __attribute__((unused))target_num, void *__attribute__((unused))ctx)
{
   struct lower_upper_ctx *sctx = ctx;
...
static void
check_new_sieve_primes(struct lower_upper_ctx *sctx, struct prime_current_block *pcb)
{
   int bit;
...
static void
compute_block(struct prime_current_block *pcb, uint32_t sieve_prime, uint32_t *offsets)
{
   int i;
   /*const unsigned char bits[] = {0x01,0x02,0x04,0x10,0x20,0x40,0x80};*/
   const unsigned char *bits = a_x_b_bitmask[pp_to_bit(sieve_prime)];

   for (i = 0; i < 8; i++) {
      if (offsets[i]  < pcb->block_size) {
         *(pcb->block + offsets[i]) |= bits[i];
         offsets[i] += sieve_prime;
      }
      offsets[i] -= pcb->block_size;
   }
}


int
lower_upper_calc_primes(struct prime_current_block *pcb, void *ctx)
{
   struct lower_upper_ctx *sctx = ctx;
   ...
   for (i = 0; i < sctx->calculated_index; i++)
      compute_block(pcb, sctx->primelist[i], &sctx->offsets[i<<3]);
